function preload() {
    this.load.image('ship', 'C:/Users/aniks/Downloads/Game/assets/spaceship_x10.png');
  ...
  function create() {
    ...
    for (let yVal = 1; yVal < 4 ; yVal++){
      for (let xVal = 1; xVal < 9; xVal++){
        gameState.enemies.create(50 * xVal, 50 * yVal, 'ship')  .setScale(.6).setGravityY(-200)
      }
    }
    ...
    this.physics.add.collider(gameState.enemies, gameState.bugRepellent, (bug,repellent) => {
      bug.destroy();
      repellent.destroy();
      gameState.scoreText.setText(`Ships Left: ${numOfTotalEnemies()}`)
    })
